...
#define SMART_LIGHT_DTID    "dt4aac877e7f95473dab59386a21976095"
...
void CloudResetConfig(bool reset_dtid)
{
	...
	if (reset_dtid)
		strncpy(cloud_config.device_type_id, SMART_LIGHT_DTID, AKC_DTID_LEN);
}

static void setAction(int brightnessPercentage)
{
    // update the cloud field value
    char tempStr[50];
    sprintf(tempStr, "{\"%s\":%d}", BRIGHTNESS_LEVEL, brightnessPercentage);
    SendMessageToCloud(tempStr);

    set_brightness_level(round((float)brightnessPercentage*PERCENT_FACTOR));
}

static void getActionParameter(cJSON* action)
{
    char tempStr[50];
    cJSON *parameters = cJSON_GetObjectItem(action, "parameters");
    if (!parameters || (parameters->type != cJSON_Object))
        return;

    brightness_level = cJSON_GetObjectItem(parameters, BRIGHTNESS_LEVEL)->valueint;

    fprintf(stderr, "\tparameters: %s, value:%d\n", BRIGHTNESS_LEVEL, brightness_level);

    // update the cloud field value
    sprintf(tempStr, "{\"%s\":%d}", BRIGHTNESS_LEVEL, brightness_level);
    SendMessageToCloud(tempStr);

    set_brightness_level(round((float)brightness_level*PERCENT_FACTOR));
}
...
static void cloud_websocket_rx_cb(void *user_data, void *result)
{
	...
	for (action = (actions)->child; action != NULL; action = action->next)
    {
        ...
        // Action "SetOn"
		if (!strncmp(name->valuestring, ACTION_SET_ON, strlen(ACTION_SET_ON)))
        {
            fprintf(stderr, "\nCLOUD ACTION: %s, set brightness to %d percent\n", ACTION_SET_ON, BRIGHTNESS_ON_PERCENTAGE);
            setAction(BRIGHTNESS_ON_PERCENTAGE);
		}
        else if (!strncmp(name->valuestring, ACTION_SET_READING, strlen(ACTION_SET_READING)))
        {
            fprintf(stderr, "\nCLOUD ACTION: %s, set brightness to %d percent\n", ACTION_SET_READING, BRIGHTNESS_READING_PERCENTAGE);
            setAction(BRIGHTNESS_READING_PERCENTAGE);
        }
        else if (!strncmp(name->valuestring, ACTION_SET_MOVIE, strlen(ACTION_SET_MOVIE)))
        {
        	fprintf(stderr, "\nCLOUD ACTION: %s, set brightness to %d percent\n", ACTION_SET_MOVIE, BRIGHTNESS_MOVIE_PERCENTAGE);
            setAction(BRIGHTNESS_MOVIE_PERCENTAGE);
        }        
        else if (!strncmp(name->valuestring, ACTION_SET_SLEEP, strlen(ACTION_SET_SLEEP)))
        {
        	fprintf(stderr, "\nCLOUD ACTION: %s, set brightness to %d percent\n", ACTION_SET_SLEEP, BRIGHTNESS_SLEEP_PERCENTAGE);
            setAction(BRIGHTNESS_SLEEP_PERCENTAGE);
	    }
        else if (!strncmp(name->valuestring, ACTION_SET_OFF, strlen(ACTION_SET_OFF)))
        {
            fprintf(stderr, "\nCLOUD ACTION: %s, set brightness to %d percent\n", ACTION_SET_OFF, BRIGHTNESS_OFF_PERCENTAGE);
            setAction(BRIGHTNESS_OFF_PERCENTAGE);
        }
        else if (!strncmp(name->valuestring, ACTION_SET_BRIGHTNESS, strlen(ACTION_SET_BRIGHTNESS)))
        {
